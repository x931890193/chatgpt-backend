// Package useragent user-agent parser, Used for web page statistics
package useragent
...
type UserAgent struct {
	...
	OS     struct {
		...
		Version string `json:"os"`
	} `json:"os"`
	Robot struct {
		Name     string `json:"robot"`
		URL      string `json:"url"`
		Producer struct {
			Name string `json:"name"`
			URL  string `json:"url"`
...
func (ua UserAgent) String() string {
	res, _ := json.Marshal(ua)
	return string(res)
}

func (ua UserAgent) Value() (driver.Value, error) {
